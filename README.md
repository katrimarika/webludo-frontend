...
A static web application for a virtual "Kalja-Kimble" boardgame. Built using Preact, Typescript, Vite and Phoenix. Made for [oniskanen's Phoenix backend](https://github.com/oniskanen/webludo-server).

## Development workflow

1. Set the socket connection url environment variable by creating a `.env.local` file with the correct url (see `.env` for an example)
1. `npm install`
1. `npm start`
